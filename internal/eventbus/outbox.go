...
func (o *Outbox) Store(ctx context.Context, tx *sqlx.Tx, subject string, envelope *commonv1.EventEnvelope) error {
	...
	now := time.Now()
	query := tx.Rebind(`INSERT INTO event_outbox (id,subject,envelope,available_at,attempts,last_error,created_at,created_by,updated_at,updated_by,version) VALUES (?,?,?,?,0,'',?,?,?,?,1)`)
	if _, err := tx.ExecContext(ctx, query, envelope.EventId, subject, data, now, now, actor.ID, now, actor.ID); err != nil {
...
func (o *Outbox) dispatchBatch(ctx context.Context) error {
	for range o.cfg.DispatchBatchSize {
		...
		if err := proto.Unmarshal(record.Envelope, &envelope); err != nil {
			if releaseErr := o.release(ctx, record.ID, record.Attempts, fmt.Errorf("decode envelope: %w", err)); releaseErr != nil {
				return errors.Join(err, releaseErr)
			}
			continue
		}
		err = Publish(ctx, o.bus, record.Subject, &envelope)
		status := "success"
		if err != nil {
			status = "error"
		}
		...
		if err != nil {
			o.logger.WarnContext(ctx, "outbox event publish failed", "event_id", record.ID, "attempt", record.Attempts+1, "dead", record.Attempts+1 >= int64(o.cfg.ConsumerMaxDeliver), "error", err)
			if releaseErr := o.release(ctx, record.ID, record.Attempts, err); releaseErr != nil {
				return errors.Join(err, releaseErr)
			}
			continue
		}
		if err := o.markPublished(ctx, record.ID); err != nil {
			return err
		}
	}
	return nil
...
func (o *Outbox) claim(ctx context.Context) (outboxRecord, bool, error) {
	...
	err := o.transactor.Within(actorCtx, nil, func(tx *sqlx.Tx) error {
		query := tx.Rebind(`SELECT id,subject,envelope,attempts,created_at FROM event_outbox WHERE published_at IS NULL AND dead_at IS NULL AND deleted_at IS NULL AND available_at<=? AND (locked_until IS NULL OR locked_until<?) ORDER BY created_at,id LIMIT 1 FOR UPDATE SKIP LOCKED`)
		now := time.Now()
