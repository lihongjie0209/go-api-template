...
func TestGetScopesTenantPrincipalInSQL(t *testing.T) {
	...
	query := regexp.QuoteMeta(`SELECT ` + recordColumns + ` FROM security_logs WHERE id=? AND deleted_at IS NULL AND tenant_id=?`)
...
func TestPageScopesTenantBeforeApplyingClientFilters(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT count(*) FROM security_logs WHERE ` + where)).
		WithArgs("tenant-1").WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(0))
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT `+recordColumns+` FROM security_logs WHERE `+where+` ORDER BY occurred_at DESC,id DESC LIMIT ? OFFSET ?`)).
