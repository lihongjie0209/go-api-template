...
| Database migrations and audit triggers | in_progress | golang-migrate supports PostgreSQL/MySQL/Kingbase directories, configured database names, PG/Kingbase schemas, startup-before-listener up migration and service-specific history tables. Compile-time DDL tests now enforce exact mandatory audit types/nullability and trigger registration; PG/Kingbase actor-bound triggers maintain fields and forbid physical deletes. Concurrent up/down and history isolation run in PostgreSQL/MySQL Testcontainers. Kingbase execution and a uniform MySQL automatic-maintenance guarantee remain open. |
...
| Tests | Unit tests cover cross-tenant get denial, filter bounds/time ordering, authoritative owner resolution and active current-context checks. GitHub CI executes PostgreSQL/MySQL lifecycle coverage under the integration build tag. |
...
| Tests | Unit tests cover database-failure cleanup, SQL tenant isolation, declared-size mismatch, durable retry persistence, key validation and provider configuration. GitHub CI executes an isolated PostgreSQL/MySQL lifecycle covering upload, filtered paging, cross-tenant denial, logical/physical deletion and cleanup state. |
...
| Actor maintenance | PostgreSQL/Kingbase transactions require a principal and call transaction-local `set_config('app.actor_id', ...)`; triggers overwrite create/update/version/delete audit data and reject physical DELETE. MySQL currently maintains these columns explicitly in repositories; a database-enforced automatic equivalent remains an open portability gap. |
| Remaining evidence | PostgreSQL/MySQL are covered by isolated Testcontainers. A licensed Kingbase CI environment is still required to execute every migration and trigger. Until that and the MySQL maintenance decision are resolved, this feature remains `in_progress`. |
...
| Durability/idempotency | Producers publish a versioned event envelope to JetStream. The durable consumer acknowledges only after an actor-bound database transaction succeeds; redelivery inserts by envelope event ID with `ON CONFLICT DO NOTHING`/`INSERT IGNORE`. Business success returns an enqueue failure so a supposedly audited mutation does not silently succeed without entering the durable stream. |
...
| Database/scale | The append-oriented table has mandatory audit/version/soft-delete columns, PostgreSQL/Kingbase audit triggers, tenant/time, actor/time, operation, resource, application, request-ID and BRIN time indexes. Partition creation, retention and archival remain part of the pending large-table policy. |
...
| Tests | Unit tests cover recursive redaction, payload bounds, duration/outcome capture, tenant SQL scoping and filter bounds. Cross-dialect durable ingestion/query integration and successful CI execution remain open evidence. |
